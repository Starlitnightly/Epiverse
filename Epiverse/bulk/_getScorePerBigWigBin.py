...
from ._utilities import getTempFileName, gtfOptions
...
def countFragmentsInRegions_worker(chrom, start, end,
    ...
    if save_data:
        _file = open(getTempFileName(suffix='.bed'), 'w+t')
...
def getScorePerBin(bigWigFiles, binLength,
    ...
    # Handle GTF options
    transcriptID, exonID, transcript_id_designator, keepExons = gtfOptions(allArgs)

    imap_res = mapReduce((bigWigFiles, stepSize, binLength, save_file),
